...
bool dfs(int d, int from, long long aa, long long bb) {    //当前深度为d,分母不能小于from,分数之和恰好是aa/bb  
    if (d == maxd) {                                       //到达了最后一层  
        if (bb%aa)return false;                            //不能整除，说明最后一项不符合埃及分数的定义，失败退出  
        v[d] = bb / aa;  
        if (better(d)) {
            memcpy(ans, v, sizeof(long long)*(d + 1));     //当前找到的解是更优的，更新ans  
        }
        return true;  
    }  
    ...
    for (int i = from;; i++) {                             //枚举分母  
        ...
        v[d] = i;  
        long long b2 = bb*i;                               //计算aa/bb-1/i，通分后，分母是bb*i，分母是aa*i-bb  
        long long a2 = aa*i - bb;  
        long long g = gcd(a2, b2);                         //计算分子，分母的最大公约数，便于约分  
